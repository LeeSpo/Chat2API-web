...
router.post('/ingest', async (ctx: Context) => {
  ...
  try {
    const adapter = createAdapter(ticket.providerType, {
      providerId: ticket.providerId,
      providerType: ticket.providerType,
      authMethods: [],
      callbackPort: 8311,
    }) as { loginWithToken?: (...args: unknown[]) => Promise<OAuthResult> }

    if (typeof adapter.loginWithToken !== 'function') {
      ctx.status = 400
      ctx.body = {
        success: false,
        error: { code: 'unsupported_provider', message: 'Provider does not support token login' },
      }
      return
    }
    ...
    if (result.success && result.credentials) {
      ...
      for (const [key, value] of Object.entries(creds)) {
        if (key === 'token' || typeof value !== 'string' || !value) continue
        extras[key] = value
      }
      result.credentials = { ...result.credentials, ...extras }
    }
