...
test('management API exposes built-in providers for docker admin credential forms', () => {
  const source = fs.readFileSync('backend/proxy/routes/management/providers.ts', 'utf8')
...
test('management API preserves built-in provider identity and credential fields when adding one from docker admin', () => {
  const routeSource = fs.readFileSync('backend/proxy/routes/management/providers.ts', 'utf8')
  const providerManagerSource = fs.readFileSync('backend/store/providers.ts', 'utf8')
