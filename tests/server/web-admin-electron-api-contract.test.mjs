...
test('web admin uses bookmarklet OAuth instead of docker in-app oauth', () => {
  ...
  assert.match(source, /\/oauth\/bookmarklet\/poll/)
  assert.match(panelSource, /Save to Chat2API|oauth.bookmarklet.dragButton/)
  assert.match(addAccountSource, /BookmarkletPanel/)
  assert.match(addAccountSource, /isDockerWebAdmin/)
  assert.match(addProviderSource, /BookmarkletPanel/)
  assert.match(addProviderSource, /isDockerWebAdmin/)
  assert.match(electronTypes, /bookmarklet:/)
